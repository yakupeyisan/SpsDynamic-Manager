...
import { joinOptions } from './pdks-position-config';
import { tableColumns } from './pdks-position-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './pdks-position-form-config';
...
@Component({
  ...
  templateUrl: './pdks-position.component.html',
  styleUrls: ['./pdks-position.component.scss']
})
export class PdksPositionComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PdksPositions`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PdksPositions/form`;
    const recid = data.PositionID || data.recid || null;
    const { PositionID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditPdksPosition' : 'AddPdksPosition',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    // Extract PositionID from selected rows
    const selectedIds: number[] = [];
    const rows = Array.isArray(event) ? event : [event];
    
    rows.forEach((row: any) => {
      const id = row.PositionID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PdksPositions/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
