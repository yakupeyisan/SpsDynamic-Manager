...
import { joinOptions } from './logs-config';
import { tableColumns } from './logs-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './logs-form-config';
...
export class LogsComponent implements OnInit {
  ...
  // Toolbar configuration - hide add, edit, delete, save buttons for logs (read-only)
  ...
  tableDataSource = (params: any) => {
    ...
    return this.http.post<any>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/Logs`, {
      page: params.page || 1,
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('Logs API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`Logs: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        ...
        this.toastr.error('Loglar yüklenirken bir hata oluştu', 'Hata');
  ...
  onTableDelete(event: any): void {
    // Not applicable for logs (read-only)
  }
