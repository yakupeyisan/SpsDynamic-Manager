...
export class MailSettingsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.apiUrl}/api/MailSettings`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.apiUrl}/api/MailSettings/form`;
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.apiUrl}/api/MailSettings/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
