...
import { joinOptions } from './mail-settings-config';
import { tableColumns } from './mail-settings-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './mail-settings-form-config';
...
export class MailSettingsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/MailSettings`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/MailSettings/form`;
    const recid = data.ID || data.recid || null;
    const { ID, recid: _, ...record } = data;
    return this.http.post(url, {
      request: { action: 'save', recid: recid, name: isEdit ? 'EditMailSetting' : 'AddMailSetting', record: record }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/MailSettings/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
