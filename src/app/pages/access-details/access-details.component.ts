...
import { joinOptions } from './access-details-config';
import { tableColumns } from './access-details-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './access-details-form-config';
...
export class AccessDetailsComponent implements OnInit {
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('AccessDetails API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`AccessDetails: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    return of({ error: true, message: 'Geçiş Yetki Raporu sadece görüntüleme amaçlıdır.' });
  };
