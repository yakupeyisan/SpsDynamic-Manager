...
import { joinOptions } from './subscription-events-config';
import { tableColumns } from './subscription-events-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './subscription-events-form-config';
...
@Component({
  ...
  templateUrl: './subscription-events.component.html',
  styleUrls: ['./subscription-events.component.scss']
})
export class SubscriptionEventsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/SubscriptionEvents`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    return of({ error: true, message: 'Abone kayıtları sadece görüntüleme amaçlıdır.' });
  };
