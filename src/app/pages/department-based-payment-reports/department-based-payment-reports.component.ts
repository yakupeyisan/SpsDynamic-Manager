// DepartmentBasedPaymentReports Component
...
import { joinOptions } from './department-based-payment-reports-config';
import { tableColumns } from './department-based-payment-reports-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './department-based-payment-reports-form-config';
...
@Component({
  selector: 'app-department-based-payment-reports',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './department-based-payment-reports.component.html',
  styleUrls: ['./department-based-payment-reports.component.scss']
})
export class DepartmentBasedPaymentReportsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    ...
    if (processedSearch && processedSearch.conditions && Array.isArray(processedSearch.conditions)) {
      ...
      processedSearch = processedSearch.conditions.map((condition: any) => {
        ...
        if (column) {
          ...
          if (searchField === 'StartDate' && operator === 'equals') {
            operator = 'between';
          } else if (searchField === 'Department') {
            // Department always uses 'begins' operator
          ...
          // For StartDate with between operator, ensure value is array
          let value = condition.value;
          if (searchField === 'StartDate' && operator === 'between') {
    ...
    return this.http.post<any>(`${environment.apiUrl}/api/CafeteriaEvents/DepartmentBasedPayments`, {
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('DepartmentBasedPaymentReports API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`DepartmentBasedPaymentReports: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        console.error('Error loading department based payment reports:', error);
