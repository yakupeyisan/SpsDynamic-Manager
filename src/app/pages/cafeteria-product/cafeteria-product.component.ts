...
import { joinOptions } from './cafeteria-product-config';
import { tableColumns } from './cafeteria-product-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-product-form-config';
...
@Component({
  selector: 'app-cafeteria-product',
  standalone: true,
  imports: [
    MaterialModule, 
    CommonModule, 
  ...
  templateUrl: './cafeteria-product.component.html',
  styleUrls: ['./cafeteria-product.component.scss']
})
export class CafeteriaProductComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProducts`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading cafeteria products:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProducts/form`;
    const recid = data.ProductID || data.recid || null;
    const { ProductID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaProduct' : 'AddCafeteriaProduct',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    rows.forEach((row: any) => {
      const id = row.ProductID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProducts/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
