// InsideOutsideReports Component
...
import { joinOptions } from './inside-outside-reports-config';
import { tableColumns } from './inside-outside-reports-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './inside-outside-reports-form-config';
...
@Component({
  selector: 'app-inside-outside-reports',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './inside-outside-reports.component.html',
  styleUrls: ['./inside-outside-reports.component.scss']
})
export class InsideOutsideReportsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    ...
    return this.http.post<any>(`${environment.apiUrl}/api/AccessEvents/InsideOutsideReports`, {
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('InsideOutsideReports API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`InsideOutsideReports: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        console.error('Error loading inside/outside reports:', error);
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    return of({ error: true, message: 'İçeridekiler/Dışarıdakiler raporu sadece görüntüleme amaçlıdır.' });
  };
