// CafeteriaApplications Component
...
import { joinOptions } from './cafeteria-applications-config';
import { tableColumns } from './cafeteria-applications-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-applications-form-config';
...
@Component({
  selector: 'app-cafeteria-applications',
  standalone: true,
  imports: [
    MaterialModule, 
    CommonModule, 
  ...
  templateUrl: './cafeteria-applications.component.html',
  styleUrls: ['./cafeteria-applications.component.scss']
})
export class CafeteriaApplicationsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaApplications`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaApplications/form`;
    const recid = data.CafeteriaApplicationID || data.recid || null;
    const { CafeteriaApplicationID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaApplication' : 'AddCafeteriaApplication',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    rows.forEach((row: any) => {
      const id = row.CafeteriaApplicationID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaApplications/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
