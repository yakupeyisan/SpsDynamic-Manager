...
import { joinOptions } from './cafeteria-accounts-config';
import { tableColumns } from './cafeteria-accounts-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-accounts-form-config';
...
@Component({
  selector: 'app-cafeteria-accounts',
  standalone: true,
  imports: [
    MaterialModule, 
    CommonModule, 
  ...
  templateUrl: './cafeteria-accounts.component.html',
  styleUrls: ['./cafeteria-accounts.component.scss']
})
export class CafeteriaAccountsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaAccounts`, {
      ...
      catchError(error => {
        console.error('Error loading cafeteria accounts:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    ...
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaAccount' : 'AddCafeteriaAccount',
        record: record
      }
    }).pipe(
      map((response: any) => {
