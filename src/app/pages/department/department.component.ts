...
import { joinOptions } from './department-config';
import { tableColumns } from './department-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './department-form-config';
...
export class DepartmentComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/Departments`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/Departments/form`;
    const recid = data.DepartmentID || data.recid || null;
    const { DepartmentID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditDepartment' : 'AddDepartment',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    // Extract DepartmentID from selected rows
    const selectedIds: number[] = [];
    const rows = Array.isArray(event) ? event : [event];
    
    rows.forEach((row: any) => {
      const id = row.DepartmentID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/Departments/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
