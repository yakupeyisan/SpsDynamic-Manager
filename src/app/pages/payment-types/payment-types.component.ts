...
import { joinOptions } from './payment-types-config';
import { tableColumns } from './payment-types-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './payment-types-form-config';
...
export class PaymentTypesComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PaymentTypes`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PaymentTypes/form`;
    ...
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditPaymentType' : 'AddPaymentType',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PaymentTypes/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
