...
export class SecureFieldsComponent implements OnInit {
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.apiUrl}/api/SecureFields/form`;
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.apiUrl}/api/SecureFields/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
