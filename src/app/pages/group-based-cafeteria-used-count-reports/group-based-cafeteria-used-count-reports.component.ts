// GroupBasedCafeteriaUsedCountReports Component
...
import { joinOptions } from './group-based-cafeteria-used-count-reports-config';
import { tableColumns } from './group-based-cafeteria-used-count-reports-table-columns';
import {
  formFields,
  formTabs,
  formLoadUrl,
  formLoadRequest,
  formDataMapper,
} from './group-based-cafeteria-used-count-reports-form-config';
import {
  DataTableComponent,
  TableColumn,
  ToolbarConfig,
...
@Component({
  selector: 'app-group-based-cafeteria-used-count-reports',
  standalone: true,
  imports: [
    MaterialModule,
    CommonModule,
  ...
  templateUrl: './group-based-cafeteria-used-count-reports.component.html',
  styleUrls: ['./group-based-cafeteria-used-count-reports.component.css'],
})
export class GroupBasedCafeteriaUsedCountReportsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    const apiUrl = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaEvents/GroupBasedCafeteriaUsedCounts`;
    return this.http
      ...
      .pipe(
        map((response: GridResponse) => {
          const records = (response.records || []).map((r: any, i: number) => ({
            ...r,
            _recid: r._recid ?? `${r.CafeteriaGroupName ?? ''}_${r.Day ?? ''}_${r.NumberOfPass ?? ''}_${i}`,
          }));
          return {
        ...
        catchError((error) => {
          console.error('Error loading group based cafeteria used count reports:', error);
