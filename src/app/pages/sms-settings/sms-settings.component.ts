...
import { joinOptions } from './sms-settings-config';
import { tableColumns } from './sms-settings-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './sms-settings-form-config';
...
export class SmsSettingsComponent implements OnInit {
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    ...
    return this.http.post(url, {
      request: { action: 'save', recid: recid, name: isEdit ? 'EditSmsSetting' : 'AddSmsSetting', record: record }
    }).pipe(
      map((response: any) => {
