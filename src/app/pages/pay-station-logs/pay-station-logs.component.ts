...
import { joinOptions } from './pay-station-logs-config';
import { tableColumns } from './pay-station-logs-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './pay-station-logs-form-config';
...
@Component({
  selector: 'app-pay-station-logs',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './pay-station-logs.component.html',
  styleUrls: ['./pay-station-logs.component.scss']
})
export class PayStationLogsComponent implements OnInit {
  ...
  // Toolbar configuration - hide add, edit, delete, save buttons for pay station logs (read-only)
  ...
  tableDataSource = (params: any) => {
    ...
    return this.http.post<any>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PayStationLogs`, {
      page: params.page || 1,
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('PayStationLogs API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`PayStationLogs: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        console.error('Error loading pay station logs:', error);
        this.toastr.error('Otomat kayıtları yüklenirken bir hata oluştu', 'Hata');
  ...
  onTableDelete(event: any): void {
    // Not applicable for pay station logs (read-only)
  }
