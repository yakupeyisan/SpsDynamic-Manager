...
import { joinOptions } from './error-logs-config';
import { tableColumns } from './error-logs-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './error-logs-form-config';
...
export class ErrorLogsComponent implements OnInit {
  ...
  // Toolbar configuration - hide add, edit, delete, save buttons for error logs (read-only)
  ...
  tableDataSource = (params: any) => {
    ...
    return this.http.post<any>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/LogError`, {
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('ErrorLogs API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`ErrorLogs: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        ...
        this.toastr.error('Hata logları yüklenirken bir hata oluştu', 'Hata');
  ...
  onTableDelete(event: any): void {
    // Not applicable for error logs (read-only)
  }
