// CafeteriaProductType Component
...
import { joinOptions } from './cafeteria-product-type-config';
import { tableColumns } from './cafeteria-product-type-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-product-type-form-config';
...
@Component({
  selector: 'app-cafeteria-product-type',
  standalone: true,
  imports: [
    MaterialModule, 
    CommonModule, 
  ...
  templateUrl: './cafeteria-product-type.component.html',
  styleUrls: ['./cafeteria-product-type.component.scss']
})
export class CafeteriaProductTypeComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProductTypes`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading cafeteria product types:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProductTypes/form`;
    const recid = data.ProductTypeID || data.recid || null;
    const { ProductTypeID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaProductType' : 'AddCafeteriaProductType',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    rows.forEach((row: any) => {
      const id = row.ProductTypeID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaProductTypes/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
