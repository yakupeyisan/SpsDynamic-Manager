...
import { joinOptions } from './pos-devices-config';
import { tableColumns } from './pos-devices-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './pos-devices-form-config';
...
@Component({
  ...
  styleUrls: ['./pos-devices.component.scss']
})
export class PosDevicesComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PosDevices`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PosDevices/form`;
    const recid = data.Id || data.recid || null;
    const { Id, recid: _, ...record } = data;
    return this.http.post(url, {
      request: { action: 'save', recid: recid, name: isEdit ? 'EditPosDevice' : 'AddPosDevice', record: record }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PosDevices/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
