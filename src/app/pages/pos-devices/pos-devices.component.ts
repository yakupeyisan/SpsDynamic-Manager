...
export class PosDevicesComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.apiUrl}/api/PosDevices`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.apiUrl}/api/PosDevices/form`;
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.apiUrl}/api/PosDevices/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
