...
import { joinOptions } from './cafeteria-group-config';
import { tableColumns } from './cafeteria-group-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-group-form-config';
...
@Component({
  ...
  templateUrl: './cafeteria-group.component.html',
  styleUrls: ['./cafeteria-group.component.scss']
})
export class CafeteriaGroupComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaGroups`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaGroups/form`;
    const recid = data.CafeteriaGroupID || data.recid || null;
    const { CafeteriaGroupID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaGroup' : 'AddCafeteriaGroup',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    rows.forEach((row: any) => {
      const id = row.CafeteriaGroupID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaGroups/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
