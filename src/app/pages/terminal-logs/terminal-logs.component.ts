...
import { joinOptions } from './terminal-logs-config';
import { tableColumns } from './terminal-logs-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './terminal-logs-form-config';
...
export class TerminalLogsComponent implements OnInit {
  ...
  // Toolbar configuration - hide add, edit, delete, save buttons for terminal logs (read-only)
  ...
  tableDataSource = (params: any) => {
    ...
    return this.http.post<any>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/LogTerminal`, {
      page: params.page || 1,
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('TerminalLogs API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`TerminalLogs: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        ...
        this.toastr.error('Terminal logları yüklenirken bir hata oluştu', 'Hata');
  ...
  onTableDelete(event: any): void {
    // Not applicable for terminal logs (read-only)
  }
