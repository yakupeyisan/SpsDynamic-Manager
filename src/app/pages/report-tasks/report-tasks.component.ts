...
import { joinOptions } from './report-tasks-config';
import { tableColumns } from './report-tasks-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './report-tasks-form-config';
...
@Component({
  ...
  styleUrls: ['./report-tasks.component.scss'],
})
  ...
  tableDataSource = (params: any): Observable<GridResponse> => {
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          console.error('Error loading report tasks:', error);
