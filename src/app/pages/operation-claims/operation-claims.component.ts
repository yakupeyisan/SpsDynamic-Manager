...
import { joinOptions } from './operation-claims-config';
import { tableColumns } from './operation-claims-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './operation-claims-form-config';
...
@Component({
  ...
  styleUrls: ['./operation-claims.component.scss']
})
export class OperationClaimsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/OperationClaims`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading operation claims:', error);
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/OperationClaims/form`;
    const recid = data.Id || data.recid || null;
    const { Id, recid: _, ...record } = data;
    return this.http.post(url, {
      request: { action: 'save', recid: recid, name: isEdit ? 'EditOperationClaim' : 'AddOperationClaim', record: record }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/OperationClaims/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
