...
export class OperationClaimsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.apiUrl}/api/OperationClaims`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.apiUrl}/api/OperationClaims/form`;
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.apiUrl}/api/OperationClaims/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
