...
import { joinOptions } from './access-events-config';
import { tableColumns } from './access-events-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './access-events-form-config';
...
@Component({
  ...
  styleUrls: ['./access-events.component.scss']
})
export class AccessEventsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/AccessEvents`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    return of({ error: true, message: 'Geçiş kayıtları sadece görüntüleme amaçlıdır.' });
  };
