...
import { joinOptions } from './cafeteria-summary-config';
import { tableColumns } from './cafeteria-summary-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-summary-form-config';
...
@Component({
  selector: 'app-cafeteria-summary',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './cafeteria-summary.component.html',
  styleUrls: ['./cafeteria-summary.component.scss']
})
export class CafeteriaSummaryComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    ...
    if (processedSearch && processedSearch.conditions && Array.isArray(processedSearch.conditions)) {
      ...
      processedSearch = processedSearch.conditions.map((condition: any) => {
        ...
        if (column) {
          ...
          if (operator === 'equals' && searchField === 'StartDate') {
            operator = 'between';
    ...
    return this.http.post<any>(`${environment.apiUrl}/api/CafeteriaEvents/Summary`, {
      ...
      map((response: any) => {
        if (response.status === 'success') {
          ...
          if (response.total !== undefined && response.total !== null) {
            ...
            console.warn('CafeteriaSummary API response missing total field. Using records length as fallback. Pagination may not work correctly.');
          ...
          if (totalValue === limit && recordsCount === limit) {
            console.warn(`CafeteriaSummary: Total (${totalValue}) equals page size (${limit}) and all records returned. This might indicate more records exist but total is incorrect.`);
          }
      ...
      catchError(error => {
        console.error('Error loading cafeteria summary:', error);
