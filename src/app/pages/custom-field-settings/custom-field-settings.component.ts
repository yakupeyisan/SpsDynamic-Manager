...
import { joinOptions } from './custom-field-settings-config';
import { tableColumns } from './custom-field-settings-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './custom-field-settings-form-config';
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    ...
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCustomFieldSettings' : 'AddCustomFieldSettings',
        record: record
      }
    }).pipe(
      map((response: any) => {
