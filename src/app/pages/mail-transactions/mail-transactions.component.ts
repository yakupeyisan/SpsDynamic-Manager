...
import { joinOptions } from './mail-transactions-config';
import { tableColumns } from './mail-transactions-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './mail-transactions-form-config';
...
@Component({
  selector: 'app-mail-transactions',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './mail-transactions.component.html',
  styleUrls: ['./mail-transactions.component.scss']
})
export class MailTransactionsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/MailTransactions`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading mail transactions:', error);
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    return of({ error: true, message: 'Mail işlemleri sadece görüntüleme amaçlıdır.' });
  };
