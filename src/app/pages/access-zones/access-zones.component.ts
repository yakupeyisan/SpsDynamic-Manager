...
import { joinOptions } from './access-zones-config';
import { tableColumns } from './access-zones-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './access-zones-form-config';
...
@Component({
  ...
  styleUrls: ['./access-zones.component.scss']
})
export class AccessZonesComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/AccessZones`, {
      ...
      catchError(error => {
        console.error('Error loading access zones:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    ...
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditAccessZone' : 'AddAccessZone',
        record: record
      }
    }).pipe(
      map((response: any) => {
