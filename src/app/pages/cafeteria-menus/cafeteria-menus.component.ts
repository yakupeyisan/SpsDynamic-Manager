...
import { joinOptions } from './cafeteria-menus-config';
import { tableColumns } from './cafeteria-menus-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-menus-form-config';
...
export class CafeteriaMenusComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaMenus`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading cafeteria menus:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaMenus/form`;
    ...
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaMenu' : 'AddCafeteriaMenu',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaMenus/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
