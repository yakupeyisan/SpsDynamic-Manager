...
import { joinOptions } from './pdks-company-config';
import { tableColumns } from './pdks-company-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './pdks-company-form-config';
...
@Component({
  ...
  templateUrl: './pdks-company.component.html',
  styleUrls: ['./pdks-company.component.scss']
})
export class PdksCompanyComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PdksCompanys`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PdksCompanys/form`;
    const recid = data.PdksCompanyID || data.recid || null;
    const { PdksCompanyID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditPdksCompany' : 'AddPdksCompany',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    // Extract PdksCompanyID from selected rows
    const selectedIds: number[] = [];
    const rows = Array.isArray(event) ? event : [event];
    
    rows.forEach((row: any) => {
      const id = row.PdksCompanyID || row.recid || row.id;
