...
export class PayStationsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.apiUrl}/api/PayStations`, {
      page: params.page || 1,
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.apiUrl}/api/PayStations/form`;
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.apiUrl}/api/PayStations/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
