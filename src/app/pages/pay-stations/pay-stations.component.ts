...
import { joinOptions } from './pay-stations-config';
import { tableColumns } from './pay-stations-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './pay-stations-form-config';
...
@Component({
  selector: 'app-pay-stations',
  standalone: true,
  imports: [MaterialModule, CommonModule, TablerIconsModule, TranslateModule, DataTableComponent],
  templateUrl: './pay-stations.component.html',
  styleUrls: ['./pay-stations.component.scss']
})
export class PayStationsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PayStations`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading pay stations:', error);
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PayStations/form`;
    const recid = data.Id || data.recid || null;
    const { Id, recid: _, ...record } = data;
    return this.http.post(url, {
      request: { action: 'save', recid: recid, name: isEdit ? 'EditPayStation' : 'AddPayStation', record: record }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/PayStations/delete`, { request: { action: 'delete', recid: selectedIds } }).subscribe({
      next: (response: any) => {
