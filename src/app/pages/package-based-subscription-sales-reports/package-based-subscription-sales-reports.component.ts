// PackageBasedSubscriptionSalesReports Component
...
import { joinOptions } from './package-based-subscription-sales-reports-config';
import { tableColumns } from './package-based-subscription-sales-reports-table-columns';
import {
  formFields,
  formTabs,
  formLoadUrl,
  formLoadRequest,
  formDataMapper,
} from './package-based-subscription-sales-reports-form-config';
import {
  DataTableComponent,
  TableColumn,
  ToolbarConfig,
...
@Component({
  selector: 'app-package-based-subscription-sales-reports',
  standalone: true,
  imports: [
    MaterialModule,
    CommonModule,
  ...
  templateUrl: './package-based-subscription-sales-reports.component.html',
  styleUrls: ['./package-based-subscription-sales-reports.component.css'],
})
export class PackageBasedSubscriptionSalesReportsComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    const apiUrl = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/SubscriptionEvents/PackageBasedSubscriptionSales`;
    return this.http
      ...
      .pipe(
        map((response: any) => {
          const records = (response.records || []).map((r: any, i: number) => ({
            ...r,
            _recid: r._recid ?? `${r.PackageName ?? ''}_${r.Day ?? ''}_${r.Qty ?? ''}_${r.DaysCount ?? ''}_${i}`,
          }));
          return {
        ...
        catchError((error) => {
          console.error('Error loading package based subscription sales reports:', error);
