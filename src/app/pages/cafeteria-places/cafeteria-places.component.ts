...
import { joinOptions } from './cafeteria-places-config';
import { tableColumns } from './cafeteria-places-table-columns';
import { formFields, formTabs, formLoadUrl, formLoadRequest, formDataMapper } from './cafeteria-places-form-config';
...
@Component({
  ...
  templateUrl: './cafeteria-places.component.html',
  styleUrls: ['./cafeteria-places.component.scss']
})
export class CafeteriaPlacesComponent implements OnInit {
  ...
  tableDataSource = (params: any) => {
    return this.http.post<GridResponse>(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaPlaces`, {
      page: params.page || 1,
      ...
      catchError(error => {
        console.error('Error loading cafeteria places:', error);
        return of({
  ...
  onSave = (data: any, isEdit: boolean): Observable<any> => {
    const url = `${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaPlaces/form`;
    const recid = data.PlaceID || data.recid || null;
    const { PlaceID, recid: _, ...record } = data;
    
    return this.http.post(url, {
      request: {
        action: 'save',
        recid: recid,
        name: isEdit ? 'EditCafeteriaPlace' : 'AddCafeteriaPlace',
        record: record
      }
    }).pipe(
      map((response: any) => {
  ...
  onTableDelete(event: any): void {
    ...
    rows.forEach((row: any) => {
      const id = row.PlaceID || row.recid || row.id;
    ...
    // Call delete API
    this.http.post(`${environment.settings[environment.setting as keyof typeof environment.settings].apiUrl}/api/CafeteriaPlaces/delete`, {
      request: {
        action: 'delete',
        recid: selectedIds
      }
